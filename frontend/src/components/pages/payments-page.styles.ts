...
tr.payment-row:hover td{background:#f8fbff}
...
.req{color:#b91c1c}
.field input,.field select{padding:9px 11px;border:1px solid var(--scim-border,#d1d5db);border-radius:10px;font-size:14px;font-family:inherit;width:100%;background:#fff;transition:border-color var(--scim-dur-fast,140ms) var(--scim-ease,ease-out),box-shadow var(--scim-dur-fast,140ms) var(--scim-ease,ease-out)}
.field input:focus-visible,.field select:focus-visible{outline:none;border-color:var(--scim-primary,#2563eb);box-shadow:0 0 0 3px color-mix(in srgb,var(--scim-primary,#2563eb) 22%, transparent)}
.form-hint{font-size:13px;color:var(--scim-text-muted,#6b7280);margin-top:4px}
