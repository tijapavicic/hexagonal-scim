...
class UserControllerAdapterTest {
    ...
    void getAllUsesConfiguredDefaultsWhenParamsMissing() {
        when(getAllUsersPort.getAll(0, 10, true))
                .thenReturn(new PagedUsers(List.of(User.createBuyer(1L, null, "a@example.com", "A")), 0, 10, 1, 1));
    ...
    void getByIdDelegatesToPort() {
        when(getUserPort.getById(7L)).thenReturn(User.createBuyer(7L, null, "seven@example.com", "Seven"));
